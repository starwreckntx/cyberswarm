...
import {
  ...
  formatStats,
} from './output/console-formatter.js';
  ...
  .action(async (options) => {
    ...
    try {
      ...
      const monitorInterval = setInterval(() => {
        ...
        const activeTasks = state.taskQueue.filter(
          t => t.status === 'EXECUTING' || t.status === 'ASSIGNED'
        );
      ...
      process.on('SIGINT', async () => {
        ...
        if (options.output) {
          ...
          const outputFile = saveSimulationResults(
            finalState.eventHistory,
            finalState.chainOfThoughts,
            {
