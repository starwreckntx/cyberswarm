...
import { Task, CyberEvent, AttackAdaptation, EventType } from '../types.js';
...
export class StrategyAdaptationAgent extends BaseAgent {
  constructor(geminiClient: GeminiClient) {
    super(
      ...
      ['adapt_strategy', 'reevaluate_targets', 'change_tactics'],
      geminiClient
    );
  }
  ...
  async executeTask(task: Task): Promise<CyberEvent> {
    ...
    try {
      this.logChainOfThought(
        1,
        "analysis",
        "Analyzing strategy adaptation request",
        `Received ${task.taskName} task. Using Gemini AI for intelligent tactical decision-making.`,
  ...
  private async adaptStrategy(task: Task): Promise<CyberEvent> {
    ...
    this.logChainOfThought(
      ...
      "analysis",
      "Situation analysis",
      `Analyzing current operational context to determine if strategy adaptation is needed`,
      { context }
    );
    ...
    const adaptation = await this.getGeminiDecision<any>(
      PROMPTS.STRATEGY_ADAPTATION(context)
    );
    ...
    if (adaptation.adaptation_needed) {
      this.logChainOfThought(
        ...
        `Adapting tactics: ${adaptation.new_strategy.tactics.join(', ')}`,
        {
  ...
  private async reevaluateTargets(task: Task): Promise<CyberEvent> {
    const context = task.details || {};
    ...
    this.logChainOfThought(
      ...
      "analysis",
      "Target reevaluation",
      `Reevaluating targets based on current situation and defensive responses`,
      { context }
    );
    ...
    this.logChainOfThought(
      ...
      `Identified ${analysis.alternative_targets?.length || 0} alternative targets`,
      { targets: analysis.alternative_targets },
      analysis.confidence
    );
    ...
    const attackAdaptation: AttackAdaptation = {
      ...
      new_techniques: analysis.evasion_techniques?.map((t: any) => t.technique) || [],
  ...
  private async changeTactics(task: Task): Promise<CyberEvent> {
    const context = task.details || {};
    ...
    this.logChainOfThought(
      ...
      "analysis",
      "Tactical changes",
      `Determining tactical adjustments to evade detection and improve effectiveness`,
      { context }
    );
    ...
    const tactics = await this.getGeminiDecision<any>(
      PROMPTS.STRATEGY_ADAPTATION({
        ...context,
        task: 'change_tactics',
      })
    ...
    this.logChainOfThought(
      ...
      `Recommended ${tactics.new_strategy?.tactics?.length || 0} tactical changes`,
