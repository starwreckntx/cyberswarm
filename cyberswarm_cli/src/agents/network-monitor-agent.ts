...
export class NetworkMonitorAgent extends BaseAgent {
  constructor(geminiClient: GeminiClient) {
    super(
      ...
      'NetworkMonitorAgent',
      ['monitor_traffic', 'detect_intrusion', 'analyze_logs'],
      geminiClient
    );
  }
  ...
  async executeTask(task: Task): Promise<CyberEvent> {
    ...
    try {
      this.logChainOfThought(
        1,
        "analysis",
        "Analyzing monitoring request",
        `Received ${task.taskName} task for ${task.target}. Using Gemini AI for intelligent threat detection.`,
  ...
  private async monitorTraffic(task: Task): Promise<CyberEvent> {
    ...
    this.logChainOfThought(
      ...
      `Monitoring network traffic on ${target} for anomalies and suspicious patterns`,
      { target }
    );
    ...
    this.logChainOfThought(
      ...
      "evaluation",
      "Traffic analysis",
      `Analyzed ${trafficData.connections} connections. Consulting Gemini AI for pattern recognition.`,
      { traffic: trafficData }
    );

    // Use Gemini for traffic analysis
    const analysis = await this.getGeminiDecision<any>(
      PROMPTS.NETWORK_MONITOR(target, trafficData)
    );
  ...
  private async detectIntrusion(task: Task): Promise<CyberEvent> {
    const target = task.target || '192.168.1.0/24';
    ...
    this.logChainOfThought(
      ...
      "analysis",
      "Intrusion detection",
      `Analyzing potential intrusion on ${target}`,
      { target, event: eventData }
    );

    await this.delay(1500, 3000);

    // Use Gemini for intrusion analysis
    const detection = await this.getGeminiDecision<any>(
      PROMPTS.NETWORK_MONITOR(target, eventData)
    );
    ...
    this.logChainOfThought(
      ...
      `Detected ${detection.intrusions_detected.length} potential intrusions`,
      { detections: detection.intrusions_detected },
      0.80
    );
    ...
    return this.emitEvent(
      EventType.MONITORING_COMPLETE,
      { status: 'no_intrusion', analysis: detection },
      'Low',
      target,
      task.taskId
    );
  ...
  private async analyzeLogs(task: Task): Promise<CyberEvent> {
    ...
    this.logChainOfThought(
      ...
      "analysis",
      "Log analysis",
      `Analyzing system logs for ${target}`,
      { target }
    );
    ...
    const logAnalysis = await this.getGeminiDecision<any>(
      `Analyze system logs for security events and anomalies. Return JSON with findings.`
    );
    ...
    this.logChainOfThought(
      ...
      `Identified patterns and potential security events in logs`,
