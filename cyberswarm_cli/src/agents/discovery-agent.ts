...
import { Task, CyberEvent, ReconData, EventType } from '../types.js';
...
export class DiscoveryAgent extends BaseAgent {
  ...
  async executeTask(task: Task): Promise<CyberEvent> {
    ...
    try {
      this.logChainOfThought(
        ...
        `Received ${task.taskName} task for target ${task.target}. Preparing reconnaissance approach using Gemini AI for intelligent decision-making.`,
  ...
  private async networkScan(task: Task): Promise<CyberEvent> {
    ...
    this.logChainOfThought(
      ...
      `Consulting Gemini AI to determine optimal scanning strategy for ${target}`,
      { strategy: "gemini_powered", target }
    );

    // Use Gemini to determine scanning strategy
    const scanStrategy = await this.getGeminiDecision<any>(
      PROMPTS.DISCOVERY_NETWORK_SCAN(target)
    );

    this.logChainOfThought(
      3,
      "decision",
      "Gemini AI recommended strategy",
      scanStrategy.reasoning,
      {
    ...
    await this.delay(
      scanStrategy.estimated_time * 800,
      scanStrategy.estimated_time * 1200
    );
    ...
    this.logChainOfThought(
      ...
      `Scan completed. Discovered ${scanStrategy.discovered_hosts.length} live hosts. Selected ${selectedHost.ip} based on confidence score and indicators.`,
      {
  ...
  private async portScan(task: Task): Promise<CyberEvent> {
    ...
    this.logChainOfThought(
      ...
      `Consulting Gemini AI for optimal port scanning approach on ${target}`,
      { target }
    );

    // Use Gemini to determine port scanning strategy
    const scanResults = await this.getGeminiDecision<any>(
      PROMPTS.DISCOVERY_PORT_SCAN(target, task.details)
    );

    this.logChainOfThought(
      3,
      "decision",
      "Gemini AI scanning strategy",
    ...
    this.logChainOfThought(
      ...
      "action",
      "Executing port scan",
      `Scanning ${scanResults.scan_results.length} ports using ${scanResults.technique}`,
      { scan_results: scanResults.scan_results }
    );
    ...
    this.logChainOfThought(
      ...
      "evaluation",
      "Port scan results",
      `Discovered ${scanResults.scan_results.filter((r: any) => r.state === 'open').length} open ports. Security assessment: ${scanResults.security_assessment.risk_level}`,
      {
        open_ports: scanResults.scan_results.filter((r: any) => r.state === 'open'),
    ...
    return this.emitEvent(
      EventType.RECON_DATA,
      reconData,
      scanResults.security_assessment.risk_level === 'critical' ? 'Critical' :
      scanResults.security_assessment.risk_level === 'high' ? 'High' : 'Medium',
      target,
      task.taskId
    );
  }
  ...
  private async serviceEnum(task: Task): Promise<CyberEvent> {
    ...
    this.logChainOfThought(
      ...
      "analysis",
      "Service enumeration",
      `Enumerating services on ${target} for ports: ${ports.join(', ')}`,
      { target, ports }
    );
    ...
    const serviceAnalysis = await this.getGeminiDecision<any>(
      PROMPTS.DISCOVERY_PORT_SCAN(target, context)
    );
    ...
    this.logChainOfThought(
      ...
      `Identified services and versions. ${serviceAnalysis.scan_results.length} services analyzed.`,
      { services: serviceAnalysis.scan_results },
      0.85
    );

    const services = serviceAnalysis.scan_results.reduce((acc: any, r: any) => {
