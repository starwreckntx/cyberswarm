...
import { KnowledgeEntry, CyberEvent, ChainOfThought } from '../types.js';
import { logger } from './logger.js';

/**
 * Load CVE database from file
 */
export function loadCVEDatabase(filePath?: string): any[] {
 ...
 */
export function saveCVEDatabase(data: any[], filePath?: string): void {
  const dbPath = filePath || path.join(process.cwd(), 'knowledge', 'cve-database.json');
 ...
 */
export function loadThreatIntelligence(filePath?: string): any {
 ...
 */
export function loadSimulationResults(filePath: string): any {
  try {
...
export function generateMarkdownReport(
  ...
  for (const [agentId, thoughts] of Object.entries(thoughtsByAgent)) {
    report += `### ${agentId}\n\n`;
    const agentThoughts = thoughts as ChainOfThought[];
 ...
 */
export function saveMarkdownReport(report: string, outputPath?: string): string {
  const timestamp = new Date().toISOString().replace(/:/g, '-').split('.')[0];
...
function groupBy<T>(items: T[], field: keyof T): Record<string, T[]> {
